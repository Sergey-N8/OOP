...
public class Cat {
    ...
    public void eat(Plate plate) {
            ...
            if (plate.decreaseFood(this.appetite)) { // Еды оказалось достаточно
                    System.out.println(this.name + " покушал в соответствии со " + 
                    "своим аппетитом " + this.appetite);
                    // Кот насытился
                    this.satiety = true;
            } else { // Еды оказалось недостаточно
                    // п.4 Считаем, что если коту мало еды в тарелке, то он её 
                    // просто не трогает,
                    // то есть не может быть наполовину сыт (это сделано для 
                    // упрощения логики программы).
                    System.out.println(this.name + " не будет есть эти крошки. " + 
                    "Еды не хватило.");
            }
    }
